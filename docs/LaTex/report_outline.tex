...
\subsection{Streamlit Dashboard Overview}

The dashboard (\texttt{dashboard.py}) serves as the comprehensive 
management interface for production quality control. Deployed at 
\url{https://csqa-cnn-dashboard.onrender.com}, it provides real-time 
monitoring, batch image analysis, AI-powered reporting, and operational 
insights to quality managers and line supervisors.

The interface is designed with three primary operational modes:

\begin{enumerate}[nosep]
    \item \textbf{Production Monitoring:} Real-time KPI tracking from 
          logged predictions in the Supabase database.
    \item \textbf{Batch Quality Inspection:} Upload and analyze multiple 
          images simultaneously via API integration.
    \item \textbf{AI Report Generation:} Generate time-based quality 
          control reports with natural language summaries and PDF export.
\end{enumerate}

\subsection{User Interface Design}

The dashboard implements a modern dark-theme interface optimised for 
extended operator viewing sessions. The design follows industry-standard 
color conventions: green (\texttt{\#10b981}) for accepted items, red 
(\texttt{\#ef4444}) for rejected items, and blue (\texttt{\#3b82f6}) 
for neutral UI elements.

Listing~\ref{lst:dashboard_theme} shows the CSS-based theming system.

\begin{lstlisting}[
  language=Python, caption={Dark theme CSS injection in the dashboard 
  (\texttt{dashboard.py}).}, label={lst:dashboard_theme},
  basicstyle=\small\ttfamily, breaklines=true,
  keywordstyle=\color{blue}, stringstyle=\color{red!60!black},
  commentstyle=\color{green!50!black}, frame=single]
st.markdown("""
...
\begin{itemize}[nosep]
    \item \textbf{Sidebar:} System status indicators (database connection, 
          Gemini AI service), refresh controls, and contextual help.
    \item \textbf{Main Panel:} Four-column KPI dashboard showing total 
          processed items, fresh count, rejection count, and overall 
          batch status.
    \item \textbf{Visualisation Layer:} Interactive Plotly charts including 
          gauge charts for rejection rate monitoring, pie charts for 
          distribution analysis, and time-series graphs for temporal trends.
\end{itemize}

\subsection{Production Monitoring Features}

The monitoring interface connects directly to the Supabase \texttt{logs} 
table and computes aggregate statistics in real time. 
Listing~\ref{lst:dashboard_metrics} demonstrates the KPI calculation.
...
# Status classification based on industrial thresholds
if dry_percentage <= 5:
    status = "GREEN OK"
elif dry_percentage <= 15:
    status = "YELLOW WARNING"
else:
    status = "RED CRITICAL"

col1, col2, col3, col4 = st.columns(4)
col1.metric("Total Processed", f"{total_count:,}")
col2.metric("Fresh (Accepted)", f"{fresh_count:,}", 
            f"{100-dry_percentage:.1f}%")
col3.metric("Dry (Rejected)", f"{dry_count:,}", 
            f"{dry_percentage:.1f}%")
col4.metric("Status", status)
\end{lstlisting}

A gauge chart provides visual feedback on the current rejection rate, 
with color-coded severity zones (green: 0--5\%, yellow: 5--15\%, 
red: $>$15\%). Time-series visualisation aggregates predictions by hour, 
enabling trend analysis and shift-based performance comparison.

\subsection{Batch Image Analysis}

The dashboard integrates with the cloud API to provide on-demand quality 
inspection of uploaded images. Operators can upload multiple images 
simultaneously for batch analysis.

Listing~\ref{lst:dashboard_batch} shows the multi-image upload and 
processing logic.

\begin{lstlisting}[
  language=Python, caption={Batch image upload and API integration 
  (\texttt{dashboard.py}).}, label={lst:dashboard_batch},
  basicstyle=\small\ttfamily, breaklines=true,
  keywordstyle=\color{blue}, stringstyle=\color{red!60!black},
  commentstyle=\color{green!50!black}, frame=single]
...
\paragraph{Analysis Results Display:}
\begin{itemize}[nosep]
    \item \textbf{Batch Summary:} Aggregated statistics showing total 
          images processed, total fruits detected, and Fresh/Dry breakdown.
    \item \textbf{Per-Image Results:} Expandable sections for each uploaded 
          image, displaying all detected fruits with individual confidence 
          scores and mini-gauge visualizations.
    \item \textbf{Quality Assessment:} Automated batch classification 
          (EXCELLENT / ACCEPTABLE / POOR) based on overall rejection rate, 
          with actionable recommendations.
\end{itemize}

The batch analysis feature uses the same detection and classification 
pipeline as the IoT simulation (Section~\ref{sec:edge}), ensuring 
consistency between automated and manual quality checks.

\subsection{Time-Based Report Generation}

The report generator module (\texttt{data\_fetcher.py}) enables flexible 
time-range filtering for production analysis. Managers can generate 
reports for predefined periods (last 24 hours, 7 days, 30 days) or 
specify custom date ranges.

Listing~\ref{lst:time_filter} demonstrates the time-range query 
implementation.

\begin{lstlisting}[
  language=Python, caption={Time-based data aggregation 
  (\texttt{data\_fetcher.py}).}, label={lst:time_filter},
  basicstyle=\small\ttfamily, breaklines=true,
  keywordstyle=\color{blue}, stringstyle=\color{red!60!black},
  commentstyle=\color{green!50!black}, frame=single]
def fetch_by_hours(self, hours: int) -> Dict:
    """Fetch data for the last N hours."""
    ...
    return self._aggregate_data(data, f"Last {hours} hours")

def _aggregate_data(self, data: list, time_period: str) -> Dict:
    """Aggregate raw logs into statistics."""
    total = len(data)
    fresh = sum(1 for r in data 
                if r.get("prediction", "").lower() == "fresh")
    dry = sum(1 for r in data 
              if r.get("prediction", "").lower() == "dry")
...
The aggregated statistics are displayed in tabular format before report 
generation, allowing operators to verify data completeness.

\subsection{AI-Powered Report Generation}

The reporting system integrates Google's Gemini 2.5 Flash model to 
transform structured production data into natural-language executive 
summaries. Reports follow a standardised template optimised for 
managerial decision-making.

Listing~\ref{lst:gemini_prompt} shows the prompt engineering strategy.

\begin{lstlisting}[
  language=Python, caption={Gemini prompt construction for quality reports 
  (\texttt{gemini\_reporter.py}).}, label={lst:gemini_prompt},
  basicstyle=\small\ttfamily, breaklines=true,
  keywordstyle=\color{blue}, stringstyle=\color{red!60!black},
  commentstyle=\color{green!50!black}, frame=single]
prompt = f"""You are a Quality Control Manager.

**Production Data for {time_period}:**
- Total Units: {total}
- Fresh (Accepted): {fresh} ({(fresh/total)*100:.1f}%)
...
Minimize prose, prioritize data visualization."""
\end{lstlisting}

The LLM constructs reports containing:
\begin{itemize}[nosep]
    \item Executive summary with status classification
    \item Tabular presentation of key quality indicators
    \item Severity-based recommendations
    \item Timestamp and report metadata
\end{itemize}

Generated reports are rendered in the dashboard using Streamlit's native 
Markdown renderer.

\subsection{PDF Export Functionality}

The PDF generation module (\texttt{pdf\_generator.py}) converts Markdown 
reports into professionally styled PDF documents using WeasyPrint. 
Reports are saved to the \texttt{reports/} directory with timestamped 
filenames.

Listing~\ref{lst:pdf_gen} shows the PDF conversion pipeline.

\begin{lstlisting}[
  language=Python, caption={Markdown-to-PDF conversion 
  (\texttt{pdf\_generator.py}).}, label={lst:pdf_gen},
  basicstyle=\small\ttfamily, breaklines=true,
  keywordstyle=\color{blue}, stringstyle=\color{red!60!black},
  commentstyle=\color{green!50!black}, frame=single]
...
The dashboard provides a download button for the generated PDF, 
enabling operators to archive reports or distribute them to stakeholders.

\subsection{Connection Status Monitoring}

The sidebar displays real-time connection status for critical external 
services (Supabase database, Gemini AI). Environment variable validation 
occurs at dashboard startup, with clear error messages and setup 
instructions for missing credentials.

\begin{lstlisting}[
  language=Python, caption={Service connection validation 
  (\texttt{dashboard.py}).}, label={lst:status_check},
  basicstyle=\small\ttfamily, breaklines=true,
  keywordstyle=\color{blue}, stringstyle=\color{red!60!black},
  commentstyle=\color{green!50!black}, frame=single]
...
This feature prevents silent failures and guides operators through 
troubleshooting when services are unavailable.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/ch3_dashboard.png}
    \caption{This interface provides real-time monitoring of date fruit processing,
    displaying key metrics such as batch status, rejection rates,
    and hourly quality trends to identify critical issues.}
    \label{fig:dashboard}
  \end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/ch3_dashboard_2.png}
    \caption{This section features tools for uploading images for AI-driven fruit
    classification and generating detailed quality control reports based on
